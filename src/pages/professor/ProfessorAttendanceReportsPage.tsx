...
export const ProfessorAttendanceReportsPage = () => {
  ...
  const meetingReports = useMemo(() => {
    return professorMeetings
      .filter(m => m.status === 'completed')
      .map(meeting => {
        const classInfo = professorClasses.find(c => c.id === meeting.classId);
  ...
  }, [professorMeetings, professorClasses, users, enrollments]);

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Relatórios dos Meus Encontros</h1>
        <p className="text-muted-foreground">Acompanhe a participação nos encontros das suas turmas</p>
      </div>
